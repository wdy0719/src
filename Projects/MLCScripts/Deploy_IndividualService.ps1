...
$settings_email_to = "Vijay.Jayaraman@microsoft.com", "yuanji@microsoft.com", "SkypeMeetingLifeRV@microsoft.com"
...
$Report = New-Module -AsCustomObject -ScriptBlock {
    ...
    function ParseTestLogs([string]$logBackupLoaction){
        ...
        foreach($backupFolder in (Get-ChildItem $logBackupLoaction -Directory)){
            [bool]$isUITest = $backupFolder.Name.Contains("UITest");
    ...
    function UploadTestReport([string]$logBackupLoaction){
        ...
        foreach($backupFolder in (Get-ChildItem $logBackupLoaction -Directory)){
            ...
            if($isUITest){
                 ...
                 #uploads trx
                    $url = [string]::Format("http://yellowtail/api/reports/project/{0}?type=functional&component={1}&variation={2}&build={3}","mlc","SchedulerAPI","Default",$bldNumber);
                    Invoke-RestMethod -Method Post -Uri $url -InFile "$trx";
            
                #upload to vijay's site
                    PostTestStatus -buildNumber $bldNumber -suitename "SchedulerAPI" -environment "Tst" -logLocation $backupFolder -trx "$trx"
            }
...
$yellowTailBuild="MlcDaily_Stress";
...
try{ 
  ...
  if(!$skipeDeploy)
   ...
   # $Report.SSDeployment=$Report.GetDeploymentStatus($ssstatus);
    ...
    $dsstatus = DeployAzureService -ccprojName "DataService" -ver "Debug" -env "Tst"
  ...
  if(!$skipeTest)
   {
    ...
    RunTests -workDir $testDir -vstestArgs "Microsoft.Skype.SkypeCast.Test.UITests.dll /InIsolation /TestCaseFilter:TestCategory!=LocalExclusive /logger:trx" -trxBackupPrefix "SchedulingUITests" -mode "Local" -browser "internet explorer" -browserVer "11_8" -platform "windows"
    #RunTests -workDir $testDir -vstestArgs "Microsoft.Skype.SkypeCast.Test.UITests.dll /InIsolation /TestCaseFilter:TestCategory!=LocalExclusive /logger:trx" -trxBackupPrefix "SchedulingUITests" -mode "Grid" -browser "firefox" -browserVer "" -platform "windows"
    #RunTests -workDir $testDir -vstestArgs "Microsoft.Skype.SkypeCast.Test.UITests.dll /InIsolation /TestCaseFilter:TestCategory!=LocalExclusive /logger:trx" -trxBackupPrefix "SchedulingUITests" -mode "Grid" -browser "chrome" -browserVer "" -platform "windows"
    
    # mobile browsers:
    #RunTests -workDir $testDir -vstestArgs "Microsoft.Skype.SkypeCast.Test.UITests.dll /InIsolation /TestCaseFilter:TestCategory!=LocalExclusive /logger:trx" -trxBackupPrefix "SchedulingUITests" -mode "Mobile" -browser "safari" -browserVer "iPhone.6" -platform "mac"
    #RunTests -workDir $testDir -vstestArgs "Microsoft.Skype.SkypeCast.Test.UITests.dll /InIsolation /TestCaseFilter:TestCategory!=LocalExclusive /logger:trx" -trxBackupPrefix "SchedulingUITests" -mode "Mobile" -browser "chrome" -browserVer "asus.Nexus.7" -platform "android"

    # Run API tests: 
    RunTests -workDir $testDir -vstestArgs "SchedulingSvcWebAPITests.dll /InIsolation /TestCaseFilter:TestCategory!=LocalExclusive /logger:trx" -trxBackupPrefix "SchedulingSvcAPITests";
    RunTests -workDir $testDir -vstestArgs "ActivationServiceWebAPITests.dll /InIsolation /TestCaseFilter:TestCategory!=LocalExclusive /logger:trx" -trxBackupPrefix "JoinActivationSvcAPITests";
